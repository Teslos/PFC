    ...
    DO i=1,Nx
     DO j=1,Ny
      !!set corner [ghost] node values around (i,j) (see finite volume text in appendix)
		...
		!! For fully explicit approach:
	    !cipjp=( BUFF(i+1,j+1) + BUFF(i,j+1) + BUFF(i,j) + BUFF(i+1,j) )/4.0d0
        !cipjm=( BUFF(i+1,j) + BUFF(i,j) + BUF(i,j-1) + BUFF(i+1,j-1) )/4.0d0
        !cimjp=( BUFF(i,j+1) + BUFF(i-1,j+1) + BUFF(i-1,j) + BUFF(i,j) )/4.0d0
        !imjm=( BUFF(i,j) + BUFF(i-1,j) + BUFF(i-1,j-1) + BUFF(i,j-1) )/4.0d0
     ...
     END DO  
    END DO  
    !re-inforce zero flux boundaries to use phi field again this iteration
	call ZERO_FLUX(PSI)
